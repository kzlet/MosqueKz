...
@Component({
  selector: 'page-changepasswordmosque',
  templateUrl: 'changepasswordmosque.html',
})
export class ChangepasswordmosquePage {
  email: any;
  apiUrl: string;
  con_code: string;
  posts: any;
  ...
  check() {
    ...
    this.apiUrl = 'http://stoke-on-trent.org/HalalHouse/mobile/checkcodemosque.php?email=' + this.email + '&confirm_code=' + this.con_code;
    ...
    this.http.get(this.apiUrl).map(res => res.json())
      .subscribe(data => {
        ...
        if(this.posts.Status === 'success' || this.posts.Status === "success")
        {
          ...
          this.navCtrl.setRoot(MosqueSignupPage);
        }
        else{
